...
vAPI.DOMFilterer = (function() {
    ...
    // :matches-attr(name="value") — match elements by attribute name/value.
    // name and value can be plain strings or /regex/.
    const PSelectorMatchesAttrTask = class {
        constructor(task) {
            this.reAttr = undefined;
            this.reValue = undefined;
            const arg = task[1] || '';
            // Parse "name" or "name"="value" format
            const match = /^"([^"]*)"(?:\s*=\s*"([^"]*)")?$/.exec(arg);
            if ( match !== null ) {
                this.reAttr = this._toRe(match[1]);
                this.reValue = match[2] !== undefined ? this._toRe(match[2]) : undefined;
            } else {
                // Fallback: treat as plain attribute name
                this.reAttr = this._toRe(arg);
            }
        }
        _toRe(s) {
            if ( s.length > 2 && s.charCodeAt(0) === 0x2F /* / */ ) {
        ...
        transpose(node, output) {
            if ( this.reAttr === undefined ) { return; }
            const attrs = node.attributes;
            if ( attrs === undefined ) { return; }
            for ( let i = 0; i < attrs.length; i++ ) {
                const attr = attrs[i];
                if ( this.reAttr.test(attr.name) === false ) { continue; }
                if ( this.reValue !== undefined && this.reValue.test(attr.value) === false ) {
                    continue;
                }
                output.push(node);
                return;
    ...
    // :matches-media(query) — match only if the media query evaluates to true.
    const PSelectorMatchesMediaTask = class {
        constructor(task) {
            this.mql = null;
            try {
    ...
    // :matches-path(path) — match only if the page URL path matches.
    const PSelectorMatchesPathTask = class {
        constructor(task) {
            this.re = null;
            const arg = task[1] || '';
            if ( arg.length > 2 && arg.charCodeAt(0) === 0x2F /* / */ ) {
                const lastSlash = arg.lastIndexOf('/');
                if ( lastSlash > 0 ) {
                    try {
                        this.re = new RegExp(arg.slice(1, lastSlash), arg.slice(lastSlash + 1));
                    } catch(e) { }
                }
            }
            if ( this.re === null ) {
                // Plain string match
                this.re = new RegExp(arg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
            }
        }
        transpose(node, output) {
            if ( this.re !== null && this.re.test(window.location.pathname + window.location.search) ) {
                output.push(node);
            }
        }
    };

    // :others() — keep sibling/cousin elements that do NOT match the base selector.
    // Used to hide everything EXCEPT the wanted content on link shortener pages.
    const PSelectorOthersTask = class {
        constructor(task) {
            this.arg = task[1] || '';
        }
        transpose(node, output) {
            // Walk up to body, collect all descendants, exclude `node` and its ancestors
            ...
            // All direct children of body that are not in the dominated chain
            if ( document.body ) {
                for ( const child of document.body.children ) {
                    if ( dominated.has(child) === false ) {
                        output.push(child);
                    }
                }
            }
    ...
    // :remove-attr(attr) — remove specified attribute(s) from matched elements.
    const PSelectorRemoveAttrTask = class {
        constructor(task) {
            this.attr = task[1] || '';
            this.re = null;
            if ( this.attr.length > 2 && this.attr.charCodeAt(0) === 0x2F ) {
                const lastSlash = this.attr.lastIndexOf('/');
                if ( lastSlash > 0 ) {
                    try {
                        this.re = new RegExp(this.attr.slice(1, lastSlash), this.attr.slice(lastSlash + 1));
                    } catch(e) { }
                }
            }
        }
    ...
    // :remove-class(class) — remove specified CSS class from matched elements.
    const PSelectorRemoveClassTask = class {
        constructor(task) {
            this.className = task[1] || '';
            this.re = null;
            if ( this.className.length > 2 && this.className.charCodeAt(0) === 0x2F ) {
                const lastSlash = this.className.lastIndexOf('/');
                if ( lastSlash > 0 ) {
                    try {
                        this.re = new RegExp(this.className.slice(1, lastSlash), this.className.slice(lastSlash + 1));
                    } catch(e) { }
                }
            }
        }
        ...
        constructor(o) {
            if ( PSelector.prototype.operatorToTaskMap === undefined ) {
                PSelector.prototype.operatorToTaskMap = new Map([
                    ...
                    [ ':remove-attr', PSelectorRemoveAttrTask ],
                    [ ':remove-class', PSelectorRemoveClassTask ],
    ...
    DOMProceduralFilterer.prototype = {
        ...
        commitNow: function() {
            ...
            for (let pselector of this.selectors.values() ) {
                ...
                } else if ( pselector.action === 'remove-attr' || pselector.action === 'remove-class' ) {
                    // Action already applied during transpose step — nothing more to do.
                } else {
