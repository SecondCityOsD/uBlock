...
FilterOriginHit.prototype = Object.create(FilterOrigin.prototype, {
    ...
    matchOrigin: {
        value: function() {
            ...
            // Handle wildcard TLD: domain.* matches domain.com, domain.co.uk, etc.
            if ( needle.endsWith('.*') ) {
                var base = needle.slice(0, -2); // "amazon"
                // haystack must contain "amazon." somewhere as a domain boundary
                var pos = haystack.indexOf(base + '.');
                if ( pos === -1 ) { return false; }
                return pos === 0 || haystack.charCodeAt(pos - 1) === 0x2E;
            }
            if ( haystack.endsWith(needle) === false ) { return false; }
            var offset = haystack.length - needle.length;
            return offset === 0 || haystack.charCodeAt(offset - 1) === 0x2E /* '.' */;
...
FilterOriginMiss.prototype = Object.create(FilterOrigin.prototype, {
    ...
    matchOrigin: {
        value: function() {
            var needle = this.hostname, haystack = pageHostnameRegister;
            // Handle wildcard TLD: ~domain.* means NOT domain.anything
            if ( needle.endsWith('.*') ) {
                var base = needle.slice(0, -2);
                var pos = haystack.indexOf(base + '.');
                if ( pos === -1 ) { return true; } // not found = miss = true
                return !(pos === 0 || haystack.charCodeAt(pos - 1) === 0x2E);
            }
...
FilterOriginHitSet.prototype = Object.create(FilterOrigin.prototype, {
    ...
    matchOrigin: {
        value: function() {
            if ( this.oneOf === null ) {
                // Separate wildcard TLD entries from normal ones
                var domains = this.domainOpt.split('|');
                var normal = [];
                var wilds = [];
                for ( var i = 0; i < domains.length; i++ ) {
                    if ( domains[i].endsWith('.*') ) {
                        wilds.push(domains[i].slice(0, -2));
                    } else {
                        normal.push(domains[i]);
                    }
                }
                this.oneOf = normal.length > 0
                    ? HNTrieBuilder.fromDomainOpt(normal.join('|'))
                    : { matches: function() { return false; } };
                this.wildcards = wilds;
            }
            if ( this.oneOf.matches(pageHostnameRegister) ) { return true; }
            // Check wildcard TLD entries
            if ( this.wildcards !== null ) {
                var haystack = pageHostnameRegister;
                for ( var j = 0; j < this.wildcards.length; j++ ) {
                    var base = this.wildcards[j] + '.';
                    var pos = haystack.indexOf(base);
                    if ( pos !== -1 && (pos === 0 || haystack.charCodeAt(pos - 1) === 0x2E) ) {
                        return true;
                    }
                }
            }
...
var FilterParser = function() {
    ...
    // Allow * for wildcard TLDs (e.g., domain=amazon.*)
...
FilterParser.prototype.parseOptions = function(s) {
    ...
    for ( var i = 0; i < opts.length; i++ ) {
        ...
        if ( /^redirect(?:-rule)?(?:=|$)/.test(opt) ) {
            ...
            // redirect= → type 1, redirect-rule= → type 2
            // Bare redirect-rule (no =) in exceptions → type 2
            if ( opt === 'redirect' || opt === 'redirect-rule' ) {
                this.redirect = opt === 'redirect' ? 1 : 2;
            } else {
        ...
        // $removeparam= : strip query parameters from URLs.
        // $removeparam=paramName    — remove specific parameter
        // $removeparam=/regex/      — remove params matching regex
        // $removeparam=             — (allow form) exception
        if ( opt.startsWith('removeparam=') ) {
            this.parseTypeOption('data', not);
            this.dataType = 'removeparam';
            this.dataStr = opt.slice(12).trim();
            continue;
        }
        // Bare $removeparam (no value): strip ALL query params (block action)
        // or cancel all removeparam rules (allow action).
        if ( opt === 'removeparam' ) {
            this.parseTypeOption('data', not);
            this.dataType = 'removeparam';
            this.dataStr = '';
        ...
        // $denyallow= : domain exception list for $removeparam and other modifiers.
        // Full semantics would require per-request domain checking against the list.
        // For now, we skip the denyallow option (filter applies to all domains,
        // which is slightly over-broad but functional).
        if ( opt.startsWith('denyallow=') ) {
            continue;
        }
        // $from= / $to= : domain source/target modifiers (modern syntax).
        // Skip for now — the filter still works, just without domain restriction.
        if ( opt.startsWith('from=') || opt.startsWith('to=') ) {
            continue;
        }
        // $method= : HTTP method filter. Skip for now.
        if ( opt.startsWith('method=') ) {
            continue;
        }
        // $urlskip= : URL extraction/chaining for tracker redirects.
        // Not implemented — skip so the base filter still applies.
        if ( opt.startsWith('urlskip=') || opt === 'urlskip' ) {
            continue;
        }
        // $uritransform= : URI transformation via regex.
        if ( opt.startsWith('uritransform=') ) {
            continue;
        }
        // $replace= : response body replacement. Cannot implement on UXP
        // without filterResponseData. Skip so other options still apply.
        ...
        // $strict3p / $strict1p : strict party matching (subset of 3p/1p).
        // Treat as equivalent to regular 3p/1p.
        if ( opt === 'strict3p' ) {
            this.parsePartyOption(false, false); // = third-party
            continue;
        }
        if ( opt === 'strict1p' ) {
            this.parsePartyOption(true, false); // = first-party
            continue;
        }
        // $match-case : case-sensitive URL matching.
        // The legacy engine is already case-insensitive by default; skip.
        if ( opt === 'match-case' ) {
            continue;
        }
        // $cname : CNAME uncloaking exception. Not available on UXP.
        if ( opt === 'cname' ) {
            continue;
        }
        // $ghide / $ehide / $shide : generic/specific cosmetic hide exceptions.
        if ( opt === 'ghide' || opt === 'ehide' || opt === 'shide' ) {
            continue;
        }
        // $all : match all resource types. Set all type bits.
        if ( opt === 'all' ) {
            this.parseTypeOption('all', false);
            continue;
        }
        // $rewrite=abp-resource:* : ABP-style rewrite (AdGuard syntax). Skip.
        if ( opt.startsWith('rewrite=') ) {
            continue;
        }
        // $redirect / $redirect-rule with priority suffix (e.g. redirect=noopjs:5).
        // The priority number after ':' is not supported; strip it.
        // (Already handled above for redirect= without priority.)
        // Handle domain= with wildcard TLDs (e.g. domain=radio.*)
        // These use the `*` TLD wildcard which the legacy parser should tolerate.
        // $blocked : modifier used in urlskip chains. Skip.
        if ( opt === 'blocked' ) {
            continue;
        }
        // $permissions= : Permissions-Policy header injection. Skip.
        if ( opt.startsWith('permissions=') ) {
            continue;
        }
        // _____ separator: used in modern filter lists as a visual separator
        // between option groups. Ignore it.
        if ( /^_+$/.test(opt) ) {
            continue;
        }
    ...
    // Redirect rules can't be exception filters (except redirect-rule).
    // @@...$redirect-rule is valid: it cancels a redirect-rule.
    // @@...$redirect= (standard redirect) is still invalid.
    if ( this.redirect === 1 && this.action !== BlockAction ) {
        this.unsupported = true;
    }
    // For @@...$redirect-rule, treat as a normal allow filter (skip the redirect).
    if ( this.redirect === 2 && this.action !== BlockAction ) {
        this.redirect = 0; // Clear redirect flag, keep as allow filter
    }
};
...
FilterContainer.prototype.matchAndFetchData = function(dataType, requestURL, out, outlog) {
    ...
    while ( i < 32 ) {
        ...
        while ( entry !== undefined ) {
            f = entry.filter;
            if ( f.dataType === dataType && f.match(url, tokenOffset) === true ) {
    ...
    while ( entry !== undefined ) {
        f = entry.filter;
        if ( f.dataType === dataType && f.match(url) === true ) {
            if ( entry.categoryBits & 0x001 ) {
                toRemove.set(f.dataStr, entry);
            } else if ( entry.categoryBits & 0x002 ) {
                toAddImportant.set(f.dataStr, entry);
