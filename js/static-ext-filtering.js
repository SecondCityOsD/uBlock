...
µBlock.staticExtFilteringEngine = (function() {
    ...
    const compileProceduralSelector = (( ) => {
        ...
        const compileAttrExpression = function(s) {
            s = s.trim();
            if ( s === '' ) { return; }
            return s;
        };

        // :remove-attr(attr) / :remove-class(class) — action operators
        const compileRemoveAttrSelector = function(s) {
            s = s.trim();
            if ( s === '' ) { return; }
            return s;
        };

        // :others() — select sibling/parent elements NOT matching the base selector
        const compileOthersExpression = function(s) {
            // Argument is empty or optional
            return s || '';
        };

        // :matches-media(media-query) — match based on @media condition
        const compileMediaQuery = function(s) {
            s = s.trim();
            if ( s === '' ) { return; }
            return s;
        };

        // :matches-path(path) — match based on page URL path
        const compilePathExpression = function(s) {
            s = s.trim();
            if ( s === '' ) { return; }
            return s;
        };
        ...
        const compileArgument = new Map([
            ...
            [ ':matches-attr', compileAttrExpression ],
            [ ':matches-css', compileCSSDeclaration ],
            [ ':matches-css-after', compileCSSDeclaration ],
            [ ':matches-css-before', compileCSSDeclaration ],
            [ ':matches-media', compileMediaQuery ],
            [ ':matches-path', compilePathExpression ],
            [ ':min-text-length', compileInteger ],
            [ ':not', compileNotSelector ],
            [ ':others', compileOthersExpression ],
            [ ':remove', compileRemoveSelector ],
            [ ':remove-attr', compileRemoveAttrSelector ],
            [ ':remove-class', compileRemoveAttrSelector ],
