...
// $removeparam= : strip query parameters from URLs before the request is sent.
// Returns a modified URL if any parameters were removed, undefined otherwise.

var applyRemoveParam = function(requestURL) {
    // Only process URLs with query strings
    if ( requestURL.indexOf('?') === -1 ) { return; }

    var dominated_params = [];
    var dominated_log = µBlock.logger.isEnabled() ? [] : undefined;

    µBlock.staticNetFilteringEngine.matchAndFetchData(
        'removeparam', requestURL, dominated_params, dominated_log
    );

    if ( dominated_params.length === 0 ) { return; }

    // Parse the URL to separate query string
    var qmark = requestURL.indexOf('?');
    if ( qmark === -1 ) { return; }

    var hash = '';
    var hashIndex = requestURL.indexOf('#', qmark);
    if ( hashIndex !== -1 ) {
        hash = requestURL.slice(hashIndex);
    }
    var base = requestURL.slice(0, qmark);
    var qs = hashIndex !== -1
        ? requestURL.slice(qmark + 1, hashIndex)
        : requestURL.slice(qmark + 1);

    if ( qs === '' ) { return; }

    var params = qs.split('&');
    var originalCount = params.length;

    // For each removeparam directive, remove matching parameters
    for ( var i = 0; i < dominated_params.length; i++ ) {
        var paramSpec = dominated_params[i];
        if ( paramSpec === '' ) {
            // Empty spec with block action = remove ALL params
            params = [];
            break;
        }
        // Check if the spec is a regex: /pattern/flags
        var isRegex = paramSpec.length > 2 &&
                      paramSpec.charCodeAt(0) === 0x2F /* / */ &&
                      paramSpec.lastIndexOf('/') > 0;
        if ( isRegex ) {
            try {
                var lastSlash = paramSpec.lastIndexOf('/');
                var reStr = paramSpec.slice(1, lastSlash);
                var reFlags = paramSpec.slice(lastSlash + 1);
                var re = new RegExp(reStr, reFlags);
    ...
    // If nothing was removed, no redirect needed
    if ( params.length === originalCount ) { return; }
...
var onBeforeRequest = function(details) {
    ...
    // $removeparam= : strip tracking query parameters before the request is sent.
    // Must run before filterRequest() so that the cleaned URL is what gets blocked/allowed.
    if ( µb.hiddenSettings.ignoreRemoveParamFilters !== true ) {
        var strippedURL = applyRemoveParam(requestURL);
        if ( strippedURL !== undefined ) {
            if ( µb.logger.isEnabled() ) {
                µb.logger.writeOne(
                    tabId,
                    'redirect',
                    { source: 'removeparam', raw: requestURL + ' ⇢ ' + strippedURL },
                    requestType,
                    requestURL,
...
var onBeforeRootFrameRequest = function(details) {
    ...
    // $removeparam= for main frame (e.g., strip ?fbclid= from page URLs)
    if ( µb.hiddenSettings.ignoreRemoveParamFilters !== true ) {
        var strippedURL = applyRemoveParam(requestURL);
        if ( strippedURL !== undefined ) {
            if ( µb.logger.isEnabled() ) {
