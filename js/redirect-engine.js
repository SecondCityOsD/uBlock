...
RedirectEngine.prototype.compileRuleFromStaticFilter = function(line) {
    ...
    for ( const option of matches[3].split(',') ) {
        if ( option.startsWith('redirect=') ) {
            ...
            // Strip priority suffix (e.g., "noopjs:5" → "noopjs")
            let colonPos = redirect.indexOf(':');
            if ( colonPos !== -1 && /^\d+$/.test(redirect.slice(colonPos + 1)) ) {
                redirect = redirect.slice(0, colonPos);
            }
            continue;
        }
        ...
        // Collect type (accept multiple — use the first one for the redirect rule).
        ...
        // Skip known non-type options that aren't handled above.
        if ( option === 'important' || option === '3p' || option === '1p' ||
             option === 'third-party' || option === 'match-case' ||
             option.startsWith('denyallow=') || option.startsWith('to=') ||
             option.startsWith('from=') || option.startsWith('method=') ||
             option === 'badfilter' || option === '' ) {
            continue;
        }
    }
    ...
    // If no type specified, default to wildcard (matches any type).
